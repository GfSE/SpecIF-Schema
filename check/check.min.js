window.ajv||(window.ajv=new Ajv({allErrors:!0}));function checkSchema(e,k){if(!k||!k.schema)throw Error("checkSchema options: a schema must be supplied");var r=ajv.compile(k.schema);return r(e)?{status:0,statusText:"SpecIF schema has been checked successfully!"}:{status:971,statusText:"SpecIF schema is violated",responseType:"text",responseText:ajv.errorsText(r.errors)}}
function checkConstraints(e,k){function r(b,f){Array.isArray(b.propertyClasses)&&("resourceClass"==f&&!b["extends"]&&1>b.propertyClasses.length&&g.push({status:976,statusText:"resource class '"+b.id+"' must have at least one property class"}),b.propertyClasses.forEach(function(a){q(e.propertyClasses,a)&&g.push({status:977,statusText:"property class '"+a.id+"' of element '"+b.id+"' must reference an item in 'propertyClasses'"})}))}function u(b,f,a,c){f.forEach(function(d){d[a]&&q(b,d[a])&&g.push({status:979,
statusText:"key '"+a+"' of item with identifier '"+d.id+"' must reference a valid "+c})})}function v(b,f,a){var c=p(e.dataTypes,b.dataType);c&&Array.isArray(f)&&(!(b.multiple||"boolean"!=typeof b.multiple&&c.multiple)&&1<f.length&&g.push({status:983,statusText:a+": neither propertyClass nor dataType allow multiple values"}),f.forEach(function(d){if(Array.isArray(c.enumeration))("string"!=typeof d||q(c.enumeration,{id:d}))&&g.push({status:984,statusText:a+": all values must be an id of the dataTypes' value enumeration"});
